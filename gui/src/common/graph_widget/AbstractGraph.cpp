#include <gui/common/graph_widget/AbstractGraph.hpp>
...
CWRUtil::Q5 AbstractGraph::widgetX(int x) const
{
	...
	CWRUtil::Q5 result = CWRUtil::toQ5(x - left) * widgetWidth / graphWidth;
	//touchgfx_printf("%d - %d - %d\n", widgetWidth.to<int>(), graphWidth.to<int>(), result.to<int>());
	return result;
}
...
{
	touchgfx_printf("ABSTRACT_DRAW_START\n");
    if (numPoints < 2)
    {
	...
	touchgfx_printf("ABSTRACT_DRAW_END\n");
    return true;
}
